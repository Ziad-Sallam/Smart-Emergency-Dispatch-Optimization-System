...
const panelStyle = {
  /* ... keep your existing panel style ... */ position: "absolute",
  top: "10px",
  left: "10px",
  background: "white",
  padding: "15px",
...
const buttonStyle = (isActive) => ({
  /* ... keep existing button style ... */
  background: isActive ? "#c20000ff" : "#f0f0f0",
...
export default function EmergencyMap({
  ...
  const visibleIncidents = useMemo(() => {
    return filterType === "all"
      ? allIncidents
      : allIncidents.filter((inc) => inc.type === filterType);
  ...
  return (
    <div className="map">
      ...
      {showPanel && <div style={panelStyle}>
        <div style={{ marginBottom: "15px" }}>
          ...
          <button
            ...
            style={buttonStyle(filterType === "all")}
          >
            All
          </button>
          <button
            onClick={() => setFilterType("FIRE")}
            style={buttonStyle(filterType === "FIRE")}
          >
            Fire 🔥
          </button>
          <button
            onClick={() => setFilterType("POLICE")}
            style={buttonStyle(filterType === "POLICE")}
          >
            Police 🚨
          </button>
          <button
            onClick={() => setFilterType("MEDICAL")}
            style={buttonStyle(filterType === "MEDICAL")}
          >
            Medical 🆘
          </button>
        </div>
        <div>
          ...
          <button
            ...
            style={buttonStyle(mapStyle.includes("positron"))}
          >
            Street
          </button>
          <button
            ...
            style={buttonStyle(mapStyle.includes("dark-matter"))}
          >
            Dark
          </button>
        </div>
      ...
      >
        ...
        {(isAddingCar || isAddingIncident || isAddingStation) && pickedLocation && (
          <Marker
          ...
          >
            <div
              style={{ fontSize: "35px", filter: "drop-shadow(0 0 5px white)" }}
            >
              📍
            </div>
          </Marker>
        ...
        {cars.map((car) => (
          ...
          >
            <div
              style={{ fontSize: "30px", transition: "transform 1.5s linear" }}
            >
        ...
        {selectedItem && (
          ...
          >
            <div style={{ padding: "5px", color: "#333" }}>
              ...
              <p style={{ margin: 0 }}>
                {selectedItem.trucks
                  ? `Available Trucks: ${selectedItem.trucks}`
                  : selectedItem.desc}
              </p>
            </div>
          </Popup>
